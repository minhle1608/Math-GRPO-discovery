...
def _extract_answer(solution_str: str) -> str | None:
    ...
    last_match = None
    for pair in re.finditer(pattern, solution_str):
        last_match = pair
    if (last_match):
...
def _validate_numbers(equation_str: str, available_numbers: List[int]) -> bool:
    ...
    """
    ### YOUR CODE HERE ###
    get_res = re.findall(r"\d+", equation_str)
    int_equation_str = []

    try:
        int_equation_str = [int(val) for val in get_res]
    except ValueError:
        return False
    
    res_map = {}
    for val in available_numbers:
        res_map[val] = res_map.get(val, 0) + 1
    
    for val in int_equation_str:
        if (res_map.get(val,0) > 0):
            res_map[val]-=1
        else:
            return False
...
def _evaluate_equation(equation_str: str) -> float | None:
    ...
    if not all(char in allowed_chars for char in equation_str):
        return None
...
def reward_fn(generated_text: str, ground_truth: Dict) -> float:
    ...
    """
    ### YOUR CODE HERE ###
    #use _extract_answer and _evaluate_equation to do it
    extracted_eq = _extract_answer(generated_text)
    if extracted_eq is None:
        return 0.0
    ...
    if not _validate_numbers(extracted_eq, numbers):
        return 0.05 + format_bonus

    result = _evaluate_equation(extracted_eq)
    if result is None:
        return 0.05 + format_bonus

    #check how close is it to the desired result, not just ignore it
...
def compute_group_normalized_advantages(
    ...
    ### YOUR CODE HERE ###
    reward_list = [reward_fn(response, ground_truth) for response, ground_truth in zip(rollout_responses,repeated_ground_truths)]

    raw_rewards = torch.tensor(reward_list, dtype=torch.float32)
    rewards_2d = raw_rewards.reshape(-1, group_size)

    group_means = rewards_2d.mean(dim = 1, keepdim = True)

    advantages = rewards_2d - group_means
...
def compute_loss(
    ...
    advantages_convert = advantages.unsqueeze(1)
    unclipped_term = advantages_convert * pi_ratio

    clipped_term = advantages_convert * torch.clamp(pi_ratio, min = 1.0 - clip_range, max = 1.0 + clip_range)

    loss_per_token = -torch.minimum(unclipped_term, clipped_term)

    with torch.no_grad():
        clip_fraction = (torch.abs(pi_ratio - 1.0) > clip_range).float().mean()
        stats = {
            "loss": loss_per_token.mean().detach(),
            "clip_fraction": clip_fraction.detach()
        }
    
    return loss_per_token, stats
    ### END YOUR CODE ###
    #return loss
...
def masked_mean(tensor: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    row_means = masked_sum / mask_count.clamp(min=1)
...
def masked_mean_drgrpo(tensor: torch.Tensor, mask: torch.Tensor, num_tokens: int) -> torch.Tensor:
    ...
    masked_sum = (tensor * mask).sum(dim=-1)
    normalized = masked_sum / max(num_tokens, 1)
